...
set part_number xc7a200tffv1156-3
...
if {[file exists ${vivado_dir}/${project_name}_${part_number}] != 0} {
	# Create a directory for old vivado projects
	if {[file exists ${vivado_dir}/${old_dir}] == 0} {
		exec mkdir ${vivado_dir}/${old_dir}
	}
	exec mv ${vivado_dir}/${project_name}_${part_number} ${vivado_dir}/${old_dir}/${project_name}_${part_number}_${current_date}
	puts "${vivado_dir}/${project_name}_${part_number} already found and moved to ${vivado_dir}/${old_dir}/${project_name}_${part_number}_${current_date}"
}

exec mkdir ${vivado_dir}/${project_name}_${part_number}

# Create project
create_project ${project_name} ${vivado_dir}/${project_name}_${part_number}
...
set_property -name "xsim.simulate.runtime" -value "10ns" -objects $obj
...
cd $savedDir
	...
	#######################################
	###   SIMULATION DPI ONLY (MODULE)  ###
	#######################################

# Create 'sim_dpi_module' fileset (if not found)
if {[string equal [get_filesets -quiet sim_dpi_module] ""]} {
  create_fileset -simset sim_dpi_module
}
current_fileset -simset [ get_filesets sim_dpi_module ]

source ${script_dir}/_dpi_module.tcl

set obj [get_filesets sim_dpi_module]
set_property "top" multiplier_dpi_TB $obj
update_compile_order -fileset sim_dpi_module


	#####################################
	###   SIMULATION DPI ONLY (CORE)  ###
	#####################################


# Create 'sim_dpi_core' fileset (if not found)
if {[string equal [get_filesets -quiet sim_dpi_core] ""]} {
  create_fileset -simset sim_dpi_core
}
current_fileset -simset [ get_filesets sim_dpi_core ]

source ${script_dir}/_dpi_core.tcl

set obj [get_filesets sim_dpi_core]
set_property "top" core_dpi_TB $obj
